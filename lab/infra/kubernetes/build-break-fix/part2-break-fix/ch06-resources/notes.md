...
Jobは、一度だけ実行されるタスクを定義するためのリソースです。
Jobは、指定された数のPodを作成し、Podが正常に完了するまで監視します。
Jobを作成してみましょう。

k8s/job/job.yml を作成したのでapplyしてみましょう。

```sh
❯ kub apply -f k8s/job/job.yml
...
date-checker   1/1           10s        28s
```

通常のPodとは違いStatusがCompletedになっていることがわかります。

```sh
❯ kub get pod
NAME                 READY   STATUS      RESTARTS   AGE
date-checker-ztll7   0/1     Completed   0          36s
```

正常に動作しているのかlogを確認してみましょう

```sh
❯ kub logs date-checker-ztll7
Fri May 29 17:12:45 UTC 2026
```

describeしてみて、StatusesでSucceededに1と表示されていることを確認できます。

```sh
❯ kub describe job date-checker
....
...
CronJobは、定期的に実行されるタスクを定義するためのリソースです。
CronJobは、指定されたスケジュールに従ってJobを作成します。

()[./image/cronjob.excalidraw.png]

まずはシンプルなCronJobを作成してみましょう。
k8s/cronjob/cronjob.yml を作成したのでapplyしてみましょう。

```sh
❯ kub apply -f k8s/cronjob/cronjob.yml
cronjob.batch/hello-cron created
```

scheduleは特定の書式に則って記述する必要があります。
- * * * * * : 毎分実行
- 0 * * * * : 毎時0分に実行
- 0 0 * * * : 毎日0時に実行
- 0 0 * * 0 : 毎週日曜日の0時に実行
- 0 0 1 * * : 毎月1日の0時に実行
- 0 0 1 1 * : 毎年1月1日の0時に実行

```sh
❯ kub get job
NAME            COMPLETIONS   DURATION   AGE
date-29667922   1/1           3s         5s
```
